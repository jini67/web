...
public class 도서시스템UI {

	public static void main(String[] args) {
			JFrame f = new JFrame();
			f.setTitle("도서시스템 - 상세정보");
			f.setSize(1200, 1200);
			...
			b1.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (id.equals("")) { // 기본형 4가지만 == 로 값 비교가 가능!
						JOptionPane.showMessageDialog(f, "id는 필수입력항목입니다.");
					}
					MemberDAO2 dao = new MemberDAO2();
					int result = dao.insert(id, pw, name, tel); // 0 or 1
					if (result == 1) {
						JOptionPane.showMessageDialog(f, "회원가입 성공!");
					} else {
						JOptionPane.showMessageDialog(f, "회원가입 실패. 다시 입력해주세요.");
					}
